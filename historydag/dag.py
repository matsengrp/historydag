...
class HistoryDag:
    ...
    def add_all_allowed_edges(
        ...
        clade_dict: Dict[FrozenSet[Label], List[HistoryDagNode]] = {
            node.clade_union(): [] for node in self.postorder()
        }
...
def history_dag_from_histories(treelist: List[HistoryDag]) -> HistoryDag:
    """Build a history DAG from a list of history DAGs which are histories."""
    # merge checks that all histories have the same leaf label set.
    dag = treelist[0].copy()
